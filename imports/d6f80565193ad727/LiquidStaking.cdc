...
access(all) contract LiquidStaking {
    ...
    access(all) fun unstakeQuickly(stFlowVault: @stFlowToken.Vault): @FlowToken.Vault {
        pre {
            // Pause check
            LiquidStakingConfig.isUnstakingPaused == false: LiquidStakingError.ErrorEncode(msg: "Unstaking is paused", err: LiquidStakingError.ErrorCode.UNSTAKE_NOT_OPEN)
    ...
    access(all) fun cashoutWithdrawVoucher(voucher: @WithdrawVoucher): @FlowToken.Vault {
        pre {
    ...
    access(all) fun migrate(delegator: @FlowIDTableStaking.NodeDelegator): @stFlowToken.Vault {
        pre {
            // Pause check
    ...
    access(all) resource interface WithdrawVoucherCollectionPublic {
        access(all) view fun getVoucherInfos(): [AnyStruct]
    ...
    access(all) resource WithdrawVoucherCollection: WithdrawVoucherCollectionPublic {
        ...
        access(FungibleToken.Withdraw) fun withdraw(uuid: UInt64): @WithdrawVoucher {
        ...
        access(all) view fun getVoucherInfos(): [AnyStruct] {
            ...
            var index = 0
            while index < self.vouchers.length {
                voucherInfos = voucherInfos.concat([
                    {
    ...
    access(all) view fun calcStFlowFromFlow(flowAmount: UFix64): UFix64 {
    ...
    access(all) view fun calcFlowFromStFlow(stFlowAmount: UFix64): UFix64 {
        let currentEpochSnapshot = DelegatorManager.borrowCurrentQuoteEpochSnapshot()
