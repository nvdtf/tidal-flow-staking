...
access(all) contract DelegatorManager {
    ...
    access(all) struct EpochSnapshot {
        ...
        access(all) view fun getCollectedDelegatorCount(): Int {
            return self.delegatorCollected.length
        }

        access(all) view fun isDelegatorCollected(uuid: UInt64): Bool {
            return self.delegatorCollected.containsKey(uuid)
        }

        access(all) view fun borrowDelegatorInfo(nodeID: String, delegatorID: UInt32): &FlowIDTableStaking.DelegatorInfo? {
            if self.delegatorInfoDict.containsKey(nodeID) == false {
                return nil
            }
        ...
        access(all) view fun getNodeIDList(): [String] {
            return self.delegatorInfoDict.keys
        }

        access(all) view fun getDelegatorIDListOnNode(nodeID: String): [UInt32] {
    ...
    access(self) view fun borrowManagedDelegator(uuid: UInt64): auth(FlowIDTableStaking.DelegatorOwner) &FlowIDTableStaking.NodeDelegator? {
        return &self.allDelegators[uuid] as auth(FlowIDTableStaking.DelegatorOwner) &FlowIDTableStaking.NodeDelegator?
    }
    ...
    access(self) view fun borrowApprovedDelegatorFromNode(_ nodeID: String): auth(FlowIDTableStaking.DelegatorOwner) &FlowIDTableStaking.NodeDelegator? {
    ...
    access(self) fun registerApprovedDelegator(_ nodeID: String, _ initialCommit: @{FungibleToken.Vault}) {
        ...
        assert(
            ...
            message: "Cannot stake to the inactive node: ".concat(nodeID)
        )
        assert(
            initialCommit.balance >= FlowIDTableStaking.getDelegatorMinimumStakeRequirement(),
            message: "Initial commitment for delegator registration < system threshold"
        )
    ...
    access(all) view fun getTotalValidStakingAmount(): UFix64 {
        let currentEpochSnapshot = self.borrowCurrentQuoteEpochSnapshot()
    ...
    access(all) view fun borrowCurrentChainEpochSnapshot(): &EpochSnapshot {
    ...
    access(all) view fun getDelegatorUUIDByID(nodeID: String, delegatorID: UInt32): UInt64? {
    ...
    access(all) view fun getApprovedNodeList(): {String: UFix64} {
    ...
    access(all) view fun getApprovedDelegatorIDs(): {String: UInt64} {
        return self.approvedDelegatorIDs
    }

    /// NodeID list that involves with migrated delegators
    /// Up to 400 nodes, do not worry about the gas-limit
    access(all) view fun getMigratedNodeIDList(): [String] {
        return self.migratedDelegatorIDs.keys
    }

    access(all) view fun getMigratedDelegatorLength(nodeID: String): Int {
        return self.migratedDelegatorIDs[nodeID]!.length
    }
    
    /// [from, to)
    access(all) view fun getSlicedMigratedDelegatorIDList(nodeID: String, from: Int, to: Int): [UInt32] {
        var upTo = to
    ...
    access(all) view fun getProtocolFeeBalance(): UFix64 {
        return self.protocolFeeVault.balance
    }

    access(all) view fun getDelegatorInfoByUUID(delegatorUUID: UInt64): FlowIDTableStaking.DelegatorInfo {
    ...
    access(all) view fun getApprovedDelegatorInfoByNodeID(nodeID: String): FlowIDTableStaking.DelegatorInfo {
    ...
    /// [from, to)
    access(all) view fun getSlicedDelegatorUUIDList(from: Int, to: Int): [UInt64] {
    ...
    access(all) view fun getTotalUnstakedVaultBalance(): UFix64 {
        return self.totalUnstakedVault.balance
    }

    access(all) view fun getDelegatorsLength(): Int {
    ...
    access(all) resource Admin {
        ...
        access(all) fun borrowProtocolFeeVault(): auth(FungibleToken.Withdraw) &FlowToken.Vault {
            return &DelegatorManager.protocolFeeVault as auth(FungibleToken.Withdraw) &FlowToken.Vault
        }
